...
router.put('/profile', [
  ...
  body('type').isIn(['Government', 'Private', 'Autonomous', 'Deemed University'])
], async (req, res) => {
  try {
...
router.post('/verify-student/:studentId', requireVerification, [
...
router.get('/statistics', async (req, res) => {
  try {
    ...
    const averageCGPA = students.reduce((sum, s) => sum + s.academicInfo.cgpa, 0) / totalStudents;
    ...
    Object.keys(departmentStats).forEach(dept => {
      departmentStats[dept].averageCGPA = departmentStats[dept].totalCGPA / departmentStats[dept].total;
      departmentStats[dept].placementRate = (departmentStats[dept].placed / departmentStats[dept].total) * 100;
