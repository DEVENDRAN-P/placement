...
router.put('/profile', [
  ...
  body('company.industry').isIn(['IT Services', 'Product', 'Consulting', 'Finance', 'Healthcare', 'Education', 'Manufacturing', 'Other']),
...
router.put('/placements/:placementId/applications/:applicationId', [
  body('status').isIn(['Screened', 'Shortlisted', 'Rejected', 'Selected', 'On Hold']),
  ...
  try {
    ...
    if (round && feedback) {
      const roundData = {
        ...
        status: feedback.includes('Passed') ? 'Passed' : feedback.includes('Failed') ? 'Failed' : 'Completed',
        score: score,
        feedback: feedback,
        date: new Date()
      };
...
router.get('/statistics', async (req, res) => {
  try {
    ...
    recruiter.statistics.successRate = totalInterviews > 0 
...
// Search for students
router.get('/search-students', requireSubscription('Premium'), async (req, res) => {
  try {
    const { 
      skills, 
      minCGPA, 
